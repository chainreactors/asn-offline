...
#   2. local pyasn loaded from ipasn.dat (supplied by external refresh)
...
# Dataset (ipasn.dat only) is supplied by an external refresh pipeline
# and mounted read-only here. iptoasn-webservice fetches its own data
# independently.
